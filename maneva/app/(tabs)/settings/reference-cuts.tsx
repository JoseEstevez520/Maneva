      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 56 }}>
        ...
        {loading ? (
          <View className="px-6 items-center py-8">
            <ActivityIndicator size="large" color={themeColors.gold.DEFAULT} />
          </View>
        ) : (
          <View className="px-6 flex-row flex-wrap justify-between gap-y-4">
            {cuts.map((uri) => (
              <View key={uri} className="w-[48%] rounded-[18px] overflow-hidden bg-surface dark:bg-surface-dark border border-border dark:border-border-dark shadow-[0_6px_16px_rgba(0,0,0,0.07)]">
                ...
                >
                  <IconClose size={14} color={themeColors.premium.white} strokeWidth={2.5} />
                </TouchableOpacity>
              </View>
            ))}
          </View>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
          className="mx-6 mt-8 h-[98px] rounded-[20px] border-2 border-dashed border-gold-border dark:border-gold-dark bg-gold-bg-soft dark:bg-gold-surface-dark flex-row items-center justify-center gap-4"
          style={{ opacity: uploading || loading ? 0.6 : 1 }}
        >
          {uploading ? (
            <ActivityIndicator size="small" color={themeColors.gold.DEFAULT} />
          ) : (
            <View className="w-12 h-12 rounded-full bg-gold items-center justify-center">
              <IconAdd size={22} color={themeColors.premium.white} strokeWidth={2.8} />
            </View>
          )}
